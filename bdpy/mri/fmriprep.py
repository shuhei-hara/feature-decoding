...
class FmriprepData(object):
    ...
    def __init__(self, datapath=None, fmriprep_version='1.2', fmriprep_dir='derivatives/fmriprep', subject=None):
    ...
    def __parse_data(self):
        ...
        for sbj in subjects:
            ...
            runs = self.__parse_session(prepdir, sbj)

            self.__data[sbj].update({'session': runs}) 
    ...
    def __parse_session(self, dpath, subject):
        # sespath = os.path.join(dpath, subject, session)
        sespath = os.path.join(dpath, subject)
        funcpath = os.path.join(sespath, 'func')

        # File name patterns
        # FIXME
        if self.__fmriprep_version == '23':
        ...
        for r in run_index:
            ...
            basedir = os.path.join(self.__fmriprep_dir, 'fmriprep', subject, 'func')
    ...
    def __get_task_event_files(self):
        for sbj, sbjdata in self.__data.items():
            for ses, sesdata in sbjdata.items():
                raw_func_dir = os.path.join(self.__datapath, sbj, 'func')
                for run in sesdata:
                    # Get run label
                    if self.__fmriprep_version == '23':
                    ...
                    # Get task event file
                    event_file_name_glob = '%s_task-*_%s_events.tsv' % (sbj, run_label)
                    ...
                    # Get bold json file
                    bold_json_file_name_glob = '%s_task-*_%s_bold.json' % (sbj, run_label)
...
def create_bdata_fmriprep(dpath, data_mode='volume_native',
    ...
    fmriprep = FmriprepData(dpath, fmriprep_version=fmriprep_version, fmriprep_dir=fmriprep_dir, subject=subject)
